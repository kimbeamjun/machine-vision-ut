...
#
# 변경사항 (명세서 v5 기준):
# - task_results.result 값: "실패"/"완료" → "fail"/"success" (영어로 통일)
...
def _stay_sec_zscore_outlier(page_logs: list[dict]) -> dict[int, float]:
    """
    체류시간 z-score 이상치 → 이상치일수록 1.0에 가까운 점수
    ...
    for pno, v in stays:
        ...
        result[pno] = float(np.clip(z / 3.0, 0.0, 1.0))
...
def calc_confusion_index(
    ...
    task_results.result 값: "success" | "fail"
    ...
    # ── w6: 태스크 실패 여부 ─────────────────────────────────────
    # result 값: "fail" | "success"
    task_fail_scores = {}
    for tr in task_results:
        task_fail_scores[str(tr["task_order"])] = 1.0 if tr["result"] == "fail" else 0.0

    task_success_total = (
        sum(1 for t in task_results if t["result"] == "success") / len(task_results)
